...
current_focus: "Site redesign and first-party fan auth shipped. Host Header Layout Fix and Host Bar Room Control are HLD (signoffs open; no tickets). Next (operator-locked): Finish the Host Chrome Extension; Discord App; Watch Party Game Mode."
major_components:
  - "SPA (apps/web) — catalog, lobby, room UI; RoomMediaEngine owns chat WS, SFU session, theater playback"
  - "Host Chrome MV3 extension (apps/host-extension) — media-tab helper; host chrome is HostTheaterButtonBar + Watch Party Settings; no capture"
