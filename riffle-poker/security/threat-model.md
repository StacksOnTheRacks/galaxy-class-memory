...
assets:
  ...
  - "Riffle play session — account and anonymous (first-party; transport TBD under session-credential-transport)"
...
trust_boundaries:
  ...
  - "Identity — first-party Riffle session; Riffle verifies session before seat-scoped ops"
...
mitigations:
  ...
  - "Riffle session (account or anonymous) is seat authority; host bind is absent by design (ADR-sit-at-table)"
  ...
  - "No ambient cross-site mutation (CSRF/SameSite/Fetch metadata or bearer-only)"
  - "Anonymous token unguessable, not in query; rotate on upgrade"
  - "WS session + topic ACL; HTTP mutations authoritative; no hidden views on public topics"
...
open_questions:
  - "session-credential-transport — cookie vs bearer for a cross-site embed iframe"
  - "csrf-mutation-channel, anonymous-upgrade-fixation, embed-frame-ancestors, ws-subscribe-auth, host-spoofing-embed — see initiative security.md"
---
