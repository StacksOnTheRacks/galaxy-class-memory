...
threats:
  ...
  - "CSRF / ambient cross-site mutation — cookie session without CSRF, SameSite, or Fetch-metadata so a hostile page drives the victim's seat"
  ...
  - "Anonymous session theft — token in query string, non-HttpOnly cookie, or XSS-readable storage; holder is the seat"
...
mitigations:
  ...
  - "Seat authority is a Riffle session (account or anonymous). Host identity, room membership, and postMessage are not sufficient for seat grants"
  - "Every hidden-view read, hole-card field, and seat-scoped move is authorized for that matchId+seatId against the bound Riffle session; fail closed"
  ...
  - "Mutating iframe→Riffle APIs use an authenticated, origin-checked channel. If cookies: SameSite + CSRF and/or Fetch metadata. If bearer: no ambient cookie mutation"
  - "Anonymous session is unguessable, not placed in query strings, and rotated on account upgrade"
  - "WS subscribe requires the same Riffle session as HTTP. Topic ACLs separate public table notify from seat-scoped channels. Push is not mutation success"
...
requirements:
  ...
  - "Seat-scoped HTTP and WS operations MUST authorize the caller as the Riffle session bound to that matchId+seatId. Host JWT, room id, and postMessage MUST NOT suffice"
  ...
  - "Cross-site framing or CSRF MUST NOT submit moves or fetch hole cards for a victim session"
  - "Session transport (cookie vs bearer) MUST be chosen with embed as a cross-site iframe. Historical riffle_play HttpOnly SameSite=Lax is not assumed to work in a third-party iframe"
  - "If WS is browser-facing, subscribe MUST prove the Riffle session and MUST enforce topic ACLs. Push MUST NOT be write authority and MUST NOT carry unauthorized hidden views"
