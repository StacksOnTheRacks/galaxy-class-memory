...
interfaces:
  ...
  - "Browser → Riffle WS — same-origin (or Riffle-origin iframe) subscribe by matchId on GET /v1/ws; first control frame { type: subscribe, matchId, authorization: Bearer <token> }; non-browser MAY send upgrade Authorization; topic table:{matchId} only; outbound { type: table.refresh, matchId, cursor }; no hidden-view bodies on public frames; token never in query, hash, or Sec-WebSocket-Protocol"
...
**WS.** Browser locus: standalone two-device play and cross-origin host embed cannot rely on same-origin lab postMessage relay. Mutations stay HTTP. Public WS frames = table cursor / refresh only (`{ type: "table.refresh", matchId, cursor }`). Seat-scoped views stay HTTP. Browser subscribe proves the same IdentityStore bearer as HTTP on the first control frame after upgrade; non-browser MAY send upgrade `Authorization`. Topic ACL is public `table:{matchId}` only. Token never in query, hash, or `Sec-WebSocket-Protocol`. Host JWT, room membership, postMessage, shared play URL, `riffle_play`, `RIFFLE_HOST_API_KEY`, and `X-Riffle-Seat-Capability` are not subscribe grants.
