...
mitigations:
  ...
  - "Host-issued seat capability (locked on #3): POST /v1/seats/capability/mint with Bearer RIFFLE_HOST_API_KEY and body { matchId, seatId, playerSubject } → opaque 32-byte CSPRNG token; SHA-256 ledger; claims jti/matchId/seatId/playerSubject/iat/exp/purpose=seat; TTL 15m; reusable until expiry; no refresh and no revocation list this ticket; client presents X-Riffle-Seat-Capability; requireSeatCapability({ matchId, seatId, token }) is the only gate; riffle_play is not sufficient; postMessage is delivery only; reject missing/expired/wrong-match/wrong-seat/wrong-purpose; never log raw tokens or the host key"
...
requirements:
  ...
  - "Before any seat-scoped Turnur operation (hidden view read, turn submit for a seat), Riffle MUST verify a host-issued capability via requireSeatCapability on header X-Riffle-Seat-Capability for that matchId+seatId (purpose=seat; riffle_play not sufficient); reject missing/expired/wrong-match/wrong-seat/wrong-purpose"
