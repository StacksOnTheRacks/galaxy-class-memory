Feature: Integrate WebSockets — match-event push
  ...
  # RiffSync chat or SFU copy; production SDK keys in the browser or play-lab iframe.

  Background:
    Given the game is authenticated to Turnur with an SDK key
  ...
  Scenario: Subscribe to an unknown match is rejected
    When the game opens a WebSocket with a valid SDK key on the connect query string
    And the game sends a subscribe message for a match that does not exist
  ...
  Scenario: Play lab WebSocket client is the server-side orchestrator
    Given a two-seat match is in play in the play lab
    When the lab opens a WebSocket subscription for the match
    Then the WebSocket client runs on the server-side lab orchestrator
    And the play-lab browser and iframe never see TURNUR_SDK_KEY
